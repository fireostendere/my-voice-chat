...
По умолчанию рация использует `F8`. Клавиша меняется прямо в окне companion и
сохраняется без перезапуска. Старый ярлык **Configure PTT key** оставлен как резервный.
