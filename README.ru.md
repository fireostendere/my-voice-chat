...
Для установщика companion доступна server-only настройка:

| Переменная          | Назначение                                                                    |
| ------------------- | ----------------------------------------------------------------------------- |
| `COMPANION_EXE_URL` | HTTPS-зеркало `LiveKitCompanionSetup.exe` вместо стандартного GitHub Release. |
...
Companion работает в фоне и не открывает главное окно. Состояние процесса и последний
лог запуска доступны через **LiveKit Companion → Status and diagnostics** в меню
«Пуск». Удалить приложение можно через **Uninstall LiveKit Companion** в том же меню
или через **Windows Settings → Apps → Installed apps**.

При первом входе в комнату Windows спросит, разрешить ли текущему сайту подключаться к
companion. Проверьте адрес и нажмите **Да**. Разрешение сохраняется локально и включает
для этого origin и рацию, и торрент-кинотеатр.

По умолчанию рация использует `F8`. Для смены клавиши откройте в меню «Пуск» папку
**LiveKit Companion** и выберите **Configure PTT key**.
