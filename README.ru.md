...
- при необходимости ставит официальный Microsoft Edge WebView2 Runtime для окна управления;
...
Ярлык **LiveKit Companion** запускает фоновый сервис и открывает собственное окно
приложения с иконкой LiveKit в панели задач. В нём можно вручную добавить origin
сайта голосового чата, выбрать активную комнату, передать magnet-ссылку или файл
`.torrent`, а также поменять глобальную PTT-клавишу. Иконка LiveKit в системном трее
повторно открывает окно или завершает сервис. PID и лог запуска доступны через
**Status and diagnostics**. Удалить приложение можно через **Uninstall LiveKit
Companion** или **Windows Settings → Apps → Installed apps**.

При первом входе Chrome или Edge может сначала запросить доступ сайта к локальной сети:
разрешите его, чтобы страница увидела приложение на этом ПК. Затем companion покажет
собственный Windows-диалог. Проверьте origin сайта и нажмите **Да**. Разрешение
сохраняется локально и включает для этого origin и рацию, и торрент-кинотеатр.
Если диалог не появился, откройте в Companion раздел **Server connection**, введите
полный адрес сайта, например `https://api.iroslyakov.com/`, и нажмите **Trust server**.
Там же можно открыть сайт или удалить ранее сохранённый origin. Это именно адрес
